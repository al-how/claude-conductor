...
/**
 * Register (or remove) the Google Maps MCP server in Claude Code's user-scope config.
 * Idempotent — safe to call every startup.
 */
export function registerGoogleMapsMcp(mapsConfig: GoogleMapsConfig, logger: Logger): void {
    const configPath = CLAUDE_CONFIG_PATH;
    const configDir = dirname(configPath);

    if (!existsSync(configDir)) {
        logger.info({ configDir }, 'Claude config directory not found — skipping Google Maps MCP registration');
        return;
    }
    ...
    if (!mapsConfig.enabled) {
        if (config.mcpServers['google-maps']) {
            delete config.mcpServers['google-maps'];
            writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
            logger.info('Google Maps MCP disabled — removed entry from Claude config');
        }
        return;
    }
    ...
    config.mcpServers['google-maps'] = {
        ...
        env: { GOOGLE_MAPS_API_KEY: mapsConfig.api_key },
    };
